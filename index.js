          ...
          msg.delete(5000)
					try {
						...
						console.error(err);
						 return msg.channel.sendEmbed(new Discord.RichEmbed()
            .setColor('0x36393E')
            .setDescription(':warning: | **1-10 Arasından Bir Cızırtı Seç.**.'));
                    }
   ...
   .setTitle(`:warning: Şuanki Ses Seviyesi: **${serverQueue.volume}**`)
    ...
    .addField('Şu anda çalınan: ' + `${serverQueue.songs[0].title}`);
		...
		if (serverQueue && serverQueue.playing) {
			...
			return msg.channel.sendEmbed(new Discord.RichEmbed()
      .setTitle("**:pause_button: Hz.Gavatın Emriyle Cızırtı Durduruldu!**")
      .setColor('RANDOM'));
		}
		...
		if (serverQueue && !serverQueue.playing) {
			...
			return msg.channel.sendEmbed(new Discord.RichEmbed()
      .setTitle("**:arrow_forward: Hz.Gavatın Emriyle Cızırtı Devam Ediyor.**")
      .setColor('RANDOM'));
		}
		return msg.channel.sendEmbed(new Discord.RichEmbed()
    .setTitle(":warning: ** | Sessizliğin Gavatı Çalıyor.**")
...
async function handleVideo(video, msg, voiceChannel, playlist = false) {
	...
	if (!serverQueue) {
      ...
      .setColor('RANDOM'))
		...
		return msg.channel.sendEmbed(new Discord.RichEmbed()
    .setTitle(`:arrow_heading_up:  **${song.title}** Adlı Cızırtı Kuyruğa Eklendi!`)
    .setColor('RANDOM'))
	}
	return undefined;
}
